...
int main(void)
{
  ...
  uint8_t rx_data; // Змінна для прийому 1 символу
  uint8_t tx_msg[] = " - OK\r\n"; // Повідомлення-відповідь
  while (1)
  {
    // Чекаємо отримання 1 байта (символу) з тайм-аутом
      if (HAL_UART_Receive(&huart1, &rx_data, 1, HAL_MAX_DELAY) == HAL_OK) 
      {
          // Відправляємо отриманий символ назад
          HAL_UART_Transmit(&huart1, &rx_data, 1, 100);
          // Додаємо текст " - OK" та перенесення рядка
