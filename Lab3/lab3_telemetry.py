...
# 1. Автоматично створюємо тестовий файл gps_log.txt на 25 рядків
# (щоб перевірити розбиття по 10: буде 2 пакети по 10 і один на 5)
...
# 2. Читаємо файл і формуємо список словників
data = []
...
# 3. Відправляємо дані пакетами по 10 рядків
chunk_size = 10
url = "https://httpbin.org/post"

for i in range(0, len(data), chunk_size):
    # Вирізаємо шматок масиву від i до i+10
    chunk = data[i:i + chunk_size]
    
    print(f"\n---> Відправка пакету з {len(chunk)} записів...")
    
    # Відправляємо POST-запит у форматі JSON
    response = requests.post(url, json=chunk)
    
    if response.status_code == 200:
        print("Успішно! Сервер отримав наступні дані (поле json):")
        # httpbin.org повертає те, що отримав. Ми дістаємо поле "json"
